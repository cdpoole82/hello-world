 ...
 function runRobot(state, robot, memory) {
   for (let turn = 0;; turn++) {
     if (state.parcels.length ==0) {
       LogOutput.logOutput += `Done in ${turn} turns`;
       break;
     }
     let action = robot(state, memory);
     state = state.move(action.direction);
     memory = action.memory;
     LogOutput.logOutput += `Moved to ${action.direction}`;
      }
   }     

   //this function returns a random selection of an array index per inputted array
   function randomPick(array) {
    let choice = Math.floor(Math.random() * array.length);
    return array[choice];
  }
  //this function returns a random destination to pick from the destinations found in the village road graph
  function randomRobot(state) {
    return {direction: randomPick(roadGraph[state.place])};
  }

   //this function seeds parcels into the Village per the defined Village Graph/locations. Can't this method be put into the VillageState class definition?
